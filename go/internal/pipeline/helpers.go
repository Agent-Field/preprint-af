...
- Put every §\todobox{...}§ in its own paragraph so the clean-paper renderer can hide it without damaging surrounding prose.
...
// ExtractTODOBoxes returns manuscript TODO bodies while respecting TeX comments,
// escaped braces, nested braces, and multiline content. Macro definitions are not
// manuscript TODO instances.
func ExtractTODOBoxes(tex string) ([]string, error) {
	clean := make([]byte, 0, len(tex))
	comment := false
	for i := 0; i < len(tex); i++ {
		c := tex[i]
		if comment {
	...
	for pos := 0; ; {
		...
		prefix := s[lineStart:start]
		if strings.Contains(prefix, `\newcommand`) || strings.Contains(prefix, `\renewcommand`) {
		...
		if depth != 0 {
			return nil, fmt.Errorf("unbalanced \\todobox at byte %d", start)
		}
		out = append(out, strings.TrimSpace(s[bodyStart:i-1]))
		pos = i
	}
	return out, nil
}
...
const (
	manuscriptTODOBegin = "<!-- BEGIN MANUSCRIPT TODOBOXES -->"
	manuscriptTODOEnd   = "<!-- END MANUSCRIPT TODOBOXES -->"
)

// SyncManuscriptTODOs mirrors unresolved source TODO boxes into a managed block
// without disturbing intake, figure, citation, or author-authored TODO entries.
func SyncManuscriptTODOs(ws Workspace) (int, error) {
	files := []string{filepath.Join(ws.PaperDir, "main.tex")}
	sections, _ := filepath.Glob(filepath.Join(ws.SectionsDir, "*.tex"))
	sort.Strings(sections)
	files = append(files, sections...)
	entries := make([]string, 0)
	seen := map[string]bool{}
	for _, path := range files {
		body := ReadText(path, 0)
		if body == "" {
			continue
		}
		boxes, err := ExtractTODOBoxes(body)
		if err != nil {
			return 0, fmt.Errorf("scan %s: %w", path, err)
		}
		rel, _ := filepath.Rel(ws.Root, path)
		for _, box := range boxes {
			key := filepath.ToSlash(rel) + "\x00" + box
			if seen[key] {
				continue
			}
			seen[key] = true
			entries = append(entries, fmt.Sprintf("- `%s`: %s", filepath.ToSlash(rel), strings.ReplaceAll(box, "\n", " ")))
		}
	}
	existing := ReadText(ws.TODOPath, 0)
	b := strings.Index(existing, manuscriptTODOBegin)
	e := strings.Index(existing, manuscriptTODOEnd)
	if (b < 0) != (e < 0) || (b >= 0 && e < b) {
		return 0, fmt.Errorf("TODO.md has malformed manuscript TODO markers")
	}
	if b >= 0 {
		existing = strings.TrimRight(existing[:b], "\n") + strings.TrimLeft(existing[e+len(manuscriptTODOEnd):], "\n")
	}
	managed := manuscriptTODOBegin + "\n"
	if len(entries) == 0 {
		managed += "- none\n"
	} else {
		managed += strings.Join(entries, "\n") + "\n"
	}
	managed += manuscriptTODOEnd + "\n"
	if strings.TrimSpace(existing) != "" {
		existing = strings.TrimRight(existing, "\n") + "\n\n"
	}
	_, err := WriteText(ws.TODOPath, existing+managed)
